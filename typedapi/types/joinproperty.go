...
// JoinProperty type.
//
// https://github.com/elastic/elasticsearch-specification/blob/8e91c0692c0235474a0c21bb7e9716a8430e8533/specification/_types/mapping/core.ts#L89-L93
...
func (s *JoinProperty) UnmarshalJSON(data []byte) error {
	...
	for {
		...
		case "relations":
			if s.Relations == nil {
				s.Relations = make(map[string][]string, 0)
			}
			...
			for key, value := range rawMsg {
				...
				case bytes.HasPrefix(value, []byte("\"")), bytes.HasPrefix(value, []byte("{")):
					...
					if err != nil {
						return fmt.Errorf("%s | %w", "Relations", err)
					}
					s.Relations[key] = append(s.Relations[key], *o)
				default:
					o := []string{}
					err := json.NewDecoder(bytes.NewReader(value)).Decode(&o)
					if err != nil {
